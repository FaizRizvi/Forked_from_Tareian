...
def main(arguments):
    ...
    features = ['cdr3_isoelectric_point', 'cdr3_instability', 'cdr3_hydrophobicity']
    ...
    for filename in args.part_tables:
        ...
        clone_data = snippets.groupby_summary(data, features,
                statistics=[np.mean, np.median],
                main_group= ['participant_label'],
                subgrouping=['specimen_tissue'],
                subgrouping2=[],
                subgrouping3=[])

        cd3_len_data.append(clone_data)
    ...
    cd3_len_data.to_csv(sys.stdout)
