...
def pybedtools_window(x, y, z, boo):
    bed_1 = pybedtools.BedTool(x)
    bed_2 = pybedtools.BedTool(y)
    
    bed_intersect = bed_1.window(bed_2, w=z)

    return bed_intersect

def parse_moods(x, y):
    column_names = ['ID', 'Motif', 'TF_pos', 'Motif_sequence']

    # Create a DF that will read in the moods input
    df_moods = pd.read_csv(x, sep="|", usecols=[0,1,2,5], header=None, names=column_names)
...
def groupby_pivot(x, y, z, a):
    df_gb = x.groupby(y).count()
