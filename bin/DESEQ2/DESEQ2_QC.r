...
# point to directory
dir <- "QC_salmon_output"

# Load sample meta sheet we will read off of the meta sheet for groupings
samples <- read.table(file.path("QC_RNA_META.csv"), header = TRUE)
...
saveRDS(dds, file ="GEUVADIS_POP_dds.rds")

res <- results(dds)
saveRDS(res, file = "GEUVADIS_POP_res.rds")
write.csv(as.data.frame(res),
          file="GEUVADIS_DEseq2_POP_res.csv")
